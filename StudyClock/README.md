...
every 5 minutes, and messages scroll on demand. No numeric seconds — the
multi-zone seconds display was tried on this hardware and reverted (see the root
`CHANGELOG.md`, "Attempted and reverted"); the colon blink is the seconds tick.
...
| `study_clock/time` | `HH:MM:SS` |
| `study_clock/date` | `DD/MMM/YY` |
| `study_clock/display` | `CLOCK` / `MESSAGE` / `DATE` / `BOOT` |
| `study_clock/rssi` | Wi-Fi RSSI, dBm |
| `study_clock/ip` | IP address |
| `study_clock/brightness` | current `0`–`15` |
...
| `study_clock/cmd/brightness` | `0`–`15` | set panel intensity |
| `study_clock/cmd/reset` | any | return to clock |
| `study_clock/cmd/show_date` | any | scroll the date now |

## Home Assistant

Auto-discovered under one **Study Clock** device: sensors (Time, Date, Display,
Wi-Fi Signal, IP) plus a **Message** text box, a **Brightness** slider, and
**Show Clock** / **Show Date** buttons. No YAML packages needed.
